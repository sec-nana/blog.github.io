...
class Viewer extends Component {
  ...
  // let viewerContainer = document.getElementById("pdfViewer").innerHTML;
    this._pdfViewer = new PDFJSViewer.PDFViewer({
      container: viewerContainer,
      eventBus: this._eventBus,
    });
    console.log("this._pdfViewer=",this._pdfViewer);
  }
  componentWillUpdate(nextProps, nextState) {
    if (this.state.doc !== nextState.doc) {
      this._pdfViewer.setDocument(nextState.doc);
      console.log("document=",this._pdfViewer);
    }
  ...
  render() {
    return (<div className="Viewer" id="viewer">
      <div className="pdfViewer" id="pdfViewer"></div>
    </div>);
  }
}
